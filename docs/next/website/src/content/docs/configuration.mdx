...
`last_workspace` toggles back to the last focused workspace by workspace id, so it survives reordering and no-ops if that workspace was closed. It is unset by default.
